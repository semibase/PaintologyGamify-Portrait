package com.paintology.lite.portrait.drawing.CameraPreview.utils;
...
public final class CameraHelper {
    ...
    public static File getOutputMediaFile(Context context, @CameraConfiguration.MediaAction int mediaAction) {
        ...
        File mediaStorageDir = KGlobal.getCameraCacheDir(true, true);
        if (mediaAction == CameraConfiguration.MEDIA_ACTION_PHOTO) {
            mediaStorageDir = KGlobal.getCameraCacheDir(true, true);
        } else if (mediaAction == CameraConfiguration.MEDIA_ACTION_VIDEO) {
//            mediaStorageDir = KGlobal.getVideoCacheDir(true, true);
        }
    ...
    public static Size getSizeWithClosestRatio(List<Size> sizes, int width, int height) {
        ...
        int targetHeight = height;
        Log.e("TAGGG", "View Height Width Print PRE TRACK> " + targetHeight);
        for (Size size : sizes) {
            if (size.getWidth() == width && size.getHeight() == height)
                return size;

            double ratio = (double) (size.getHeight() * 2) / size.getWidth();

            if (Math.abs(ratio - targetRatio) < MIN_TOLERANCE) MIN_TOLERANCE = ratio;
            else continue;

            if (Math.abs(size.getHeight() - targetHeight) < minDiff) {
                Log.e("TAGGG", "View Height Width Print PRE SET 1> " + size.getWidth() + " <H> " + size.getHeight());
                optimalSize = size;
                minDiff = Math.abs(size.getHeight() - targetHeight);
            }
        }
        ...
        if (optimalSize == null) {
            ...
            for (Size size : sizes) {
                if (Math.abs(size.getHeight() - targetHeight) < minDiff) {
                    Log.e("TAGGG", "View Height Width Print PRE SET 2> " + size.getWidth() + " <H> " + size.getHeight());
                    optimalSize = size;
                    minDiff = Math.abs(size.getHeight() - targetHeight);
                }
            }
        }
        //optimalSize.setHeight(1920);
        Log.e("TAGGG", "View Height Width Print PRE 3> " + optimalSize.getWidth() + " HH " + optimalSize.getHeight());
        return optimalSize;
    }
