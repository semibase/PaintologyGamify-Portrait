package com.paintology.lite.portrait.drawing.painting;
...
import com.paintology.lite.portrait.drawing.bezier.QuadCurve;
...
public class PaintingTemp implements Serializable {
    ...
    private ArrayList<Stroker> mHisotryStrokeListOfBaseSnapShot;
    ...
    public int redoClicks;
    private int undoClicks;

    public PaintingTemp(Context pContext) {
    ...
    public Rect redoStroke() {
        if (mDeletedStrokes.isEmpty()) {
            ...
            if (redoClicks <= -1) {
                redoClicks = -1;
//                try {
//                    Paintor.obj_interface.clearAddRemoveStrokeInRedoList();
    ...
    public void showPaintingOnCanvas(Canvas pCanvas, Rect pRect, boolean pBoolean) {
        ...
        if (mDrawingCanvasVisible) {
            ...
            if (mComposeBitmap != null && !mComposeBitmap.isRecycled())
                pCanvas.drawBitmap(mComposeBitmap, 0.0F, 0.0F, null);
            pCanvas.restore();
            clearCanvas(mComposeCanvas, pRect);
        } else {
